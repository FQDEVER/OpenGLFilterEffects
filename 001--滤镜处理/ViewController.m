...
//  001--滤镜处理
//
//  Created by xxxx on 2019/4/23.
//  Copyright © 2019年 xxxx. All rights reserved.
//
...
- (void)setupFilterBar {
    ...
    NSArray *dataSource = @[@"无",@"灰度",@"颠倒",@"马1",@"马2",@"马3",@"缩放",@"灵魂出窍",@"抖动",@"闪白",@"毛刺",@"幻觉"];
...
- (void)filterInit {
    ...
    //6.获取处理的图片路径
    NSString *imagePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"AAAAA.jpg"];
    //读取图片
...
- (void)setupMosaicShaderProgram {
    [self setupShaderProgramWithName:@"MosaicLearn"];
    
}

// 六边形马赛克滤镜着色器程序
- (void)setupHexagonMosaicShaderProgram {
    [self setupShaderProgramWithName:@"MosaicLearn2"];
}

// 三角形马赛克滤镜着色器程序
- (void)setupTriangularMosaicShaderProgram {
    [self setupShaderProgramWithName:@"MosaicLearn3"];
}
...
//闪白
-(void)setupWhite{
    [self setupShaderProgramWithName:@"White"];
}
